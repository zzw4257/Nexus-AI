    ...
    return (
        <div className="model-card">
            <div className="model-card-header">
                ...
                <span className={`model-status ${isRunning ? 'running' : 'stopped'}`}>
                    {model.status}
                </span>
            </div>
            ...
            <div className="model-actions">
                ...
                <button
                    ...
                    disabled={isRunning || model.isProcessing}
                >
                    Delete
                </button>
            </div>
...
const ModelsPage = () => {
    ...
    const fetchModels = useCallback(async () => {
        ...
        // Add an `isProcessing` flag for UI feedback
    ...
    const handleAction = useCallback(async (action, modelName) => {
        // Set processing state for the specific model
        setModels(prev => prev.map(m => m.name === modelName ? { ...m, isProcessing: true } : m));

        switch(action) {
            case 'start':
                await ollama.startModel(modelName);
                break;
            case 'stop':
                await ollama.stopModel(modelName);
                break;
            case 'delete':
        ...
        // Refetch all models to get the latest state
        await fetchModels();
    }, [fetchModels]);
    ...
    const handlePullModel = async () => {
        ...
        setPullTarget('');
        setIsPulling(false);
        await fetchModels(); // Refresh list after pull
    };
        ...
        <div className="models-page-container">
            ...
            {isLoading ? (
                ...
                <div className="model-grid">
                    {models.map(model => (
                        <ModelCard key={model.name} model={model} onAction={handleAction} />
                    ))}
                </div>
            )}
        </div>
